...
namespace lab 
{
    ...
    public:
        ...
        LocalAudioInputProvider(unsigned channelCount)
            : m_sourceBus(channelCount, AudioNode::ProcessingSizeInFrames)  // FIXME: handle non-stereo local input.
        {
...
AudioDestinationNode::AudioDestinationNode(AudioContext * context, unsigned channelCount, float sampleRate) : m_context(context), m_sampleRate(sampleRate), m_currentSampleFrame(0)
{
