...
public:
    ...
    void configure(const AudioStreamBasicDescription& outDesc, UInt32 bufferSize)
    {
        ...
        m_audioBus = new AudioBus(NumDefaultOutputChannels(), bufferSize, true);
...
AudioDestinationMac::AudioDestinationMac(AudioIOCallback& callback, unsigned numberOfOutputChannels, float sampleRate)
    : m_outputUnit(0)
    , m_callback(callback)
    , m_renderBus(numberOfOutputChannels, kBufferSize, false)
